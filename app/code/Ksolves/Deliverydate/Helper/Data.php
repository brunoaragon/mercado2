...
class Data extends \Magento\Framework\App\Helper\AbstractHelper
{
    ...
    {
        return "Data de Entrega/Retirada";
    }
    ...
    {
        return "Horário de Entrega/Retirada";
    }
    ...
    public function getAddTimeSlotOption()
    {
        ...
        $_dates = array();
        foreach ($disableDateOption as $_value) {
            $timezone = $this->getConfigGeneral('ksolves_deliverydate_date_timezone', null);

            if(isNull($timezone)){
                //date_default_timezone_set('America/Sao_Paulo');
            }

            $agenda = $_value['add_end_time'];
            $agenda = str_replace("AM", " AM", $agenda);
            $agenda = str_replace("PM", " PM", $agenda);

            $horario = date('h:i A');

            //if (strtotime(date($agenda)) > (strtotime(date($horario))) + 60*30) {
                $disable_timeSlot = $_value['add_start_time'] . ' - ' . $_value['add_end_time'];
                if (!isset($disable_timeSlot)) continue;
                array_push($_dates, $disable_timeSlot);
            //}
    ...
    {
        return false; //$this->unserialize($this->getConfigTimeSlot('ksolves_deliverydate_calendar_timeslot_disable_timeslot', $store));
    }
