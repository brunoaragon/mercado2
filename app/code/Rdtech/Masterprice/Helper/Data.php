...
namespace Rdtech\Masterprice\Helper;
...
class Data extends AbstractHelper
{
    ...
    {
        return boolval($this->scopeConfig->getValue('rdtech/general/enable_price', \Magento\Store\Model\ScopeInterface::SCOPE_STORE));
    }
    ...
    public function getPriceByTablePrice($_product, $_customer)
    {
        ...
        if ($this->isLogged() && $priceTablesCustomers[0]['price_table_id'] > 0) {
            ...
            $priceTableLink = $this->scopeConfig->getValue('rdtech/pricetable/defaulttable', \Magento\Store\Model\ScopeInterface::SCOPE_STORE);
        }
        ...
        $objectManager = \Magento\Framework\App\ObjectManager::getInstance();
        $pricetable = $objectManager->create('\Rdtech\Masterprice\Model\Pricetable');
        ...
        $objectManager = \Magento\Framework\App\ObjectManager::getInstance();
        $priceproduct = $objectManager->create('\Rdtech\Masterprice\Model\Pricetableproduct');
    ...
    {
        $objectManager = \Magento\Framework\App\ObjectManager::getInstance();
        $pricerule = $objectManager->create('\Rdtech\Masterprice\Model\Pricetablerule');
        $pricelink = $objectManager->create('\Rdtech\Masterprice\Model\Pricetablelink');
    ...
    {
        return boolval($this->scopeConfig->getValue('rdtech/general/enable_discount', \Magento\Store\Model\ScopeInterface::SCOPE_STORE));
    }
    ...
    public function DiscountItem($_product)
    {
        if ($this->isLogged() && $_product->getId()) {           
            ...
            $newspecialprice = floatval($_price - (($_price * $_percentage) / 100));            

            if($_product->getPrice() > $newspecialprice && $_percentage > 0){
    ...
    {
        $objectManager = \Magento\Framework\App\ObjectManager::getInstance();
        $discountrule = $objectManager->create('\Rdtech\Masterprice\Model\Discountrule');
        $discount = $objectManager->create('\Rdtech\Masterprice\Model\Discount');
    ...
    {
        return boolval($this->scopeConfig->getValue('rdtech/general/enable_tax', \Magento\Store\Model\ScopeInterface::SCOPE_STORE));
    }
    ...
    public function TaxItem($_product)
    {
        if ($_product->getId()) {
            $objectManager = \Magento\Framework\App\ObjectManager::getInstance();
            $taxModel = $objectManager->create('Rdtech\Masterprice\Model\Tax');
    ...
    {        
        $objectManager = \Magento\Framework\App\ObjectManager::getInstance();
        $taxes = $objectManager->create('\Rdtech\Masterprice\Model\Taxlink');
        $_taxes = $taxes->getCollection()->addFieldToFilter('code',$tax)->addFieldToFilter('is_active', 1);

        $taxesRule = $objectManager->create('\Rdtech\Masterprice\Model\Taxrule');
